...
void StarterGuild::OnPlayerLogin(Player* player)
{
    if (sConfigMgr->GetOption<bool>("StarterGuild.Enable", true))
        if (sConfigMgr->GetOption<bool>("StarterGuild.Announce", true))
            ChatHandler(player->GetSession()).SendSysMessage("Dieser Server verwendet das |cff4CFF00StarterGuild |rModul.");
}
...
void StarterGuild::addPlayerToGuild(Player* player)
{
    ...
    if (guild)
    {
        if (guild->GetMemberCount() <= guildMemberCount)
        {
            ...
            ChatHandler(player->GetSession()).SendSysMessage("CoYa kann keine weiteren Mitglieder mehr aufnehmen.");
    }
    else
        ChatHandler(player->GetSession()).SendSysMessage("CoYa existiert nicht.");
}
