...
local ENABLE_ACCOUNTWIDE_MONEY = true

local ANNOUNCE_ON_LOGIN = false
local ANNOUNCEMENT = "Dieser Server verwendet das |cFF00B0E8Accountweite-Gold|r Lua-Skript."
...
local function MaybeWarnNearGoldCap(player)
    ...
    if money >= GOLD_WARNING_THRESHOLD then
        ...
        player:SendBroadcastMessage(string.format(
            "|cFF00B0E8Accountweites Gold – Hinweis:|r Du hast das Gold-Softcap von %dg erreicht. " ..
            "Zusätzlich verdientes Gold wird automatisch in deiner Accountbank gespeichert.",
...
local function AddDeltaToAccountMoney(accountId, delta)
    ...
    local sql = string.format([[
        UPDATE accountwide_money
        SET money = GREATEST(0, money + (%d))
        WHERE accountId = %d
    ]], delta, accountId)
...
local function HandleAccountBalance(player, command)
    ...
    player:SendBroadcastMessage("|cFF00B0E8Accountweites Gold|r")
	player:SendBroadcastMessage("   Accountbank:    |cffffd000" .. FormatMoney(accountTotal) .. "|r")
	player:SendBroadcastMessage("   Charakter:      |cffffd000" .. FormatMoney(charMoney) .. "|r")
...
RegisterPlayerEvent(42, function(_, player, msg)
    msg = msg:lower()
    if msg == "accountbalance" or msg == "accountmoney" or msg == "accountbank" or msg == "kontostand" or msg == "accountgold" then
